...
class ShortPathMng(Node):
    ...
    def __init__(self, resolution=8, shortPathMethod='DIJKSTRA', isLocalPlanner=False, inflate_radius=0.3):
    ...
    def inflate_map(self, map, map_resolution):
        ...
        new_inflated_map = [[point for point in row] for row in map]
        inflate_points = int(self.inflate_radius / map_resolution)

        for row_idx, row in enumerate(map):
            for point_idx, point in enumerate(row):
                if point == self.MAP_OBSTACLE_VALUE:
                    # Inflate row
                    min_row_idx = max(point_idx-inflate_points,0)
                    max_row_idx = min(point_idx+inflate_points+1, len(row))
                    for inflated_point_idx in range(min_row_idx, min_row_idx+len(row[min_row_idx:max_row_idx])):
                        new_inflated_map[row_idx][inflated_point_idx] = self.MAP_OBSTACLE_VALUE

                    # Inflate col
                    min_col_idx = max(row_idx-inflate_points,0)
                    max_col_idx = min(row_idx+inflate_points+1, len(map))
                    for inflated_col_idx in range(min_col_idx, min_col_idx+len(map[min_col_idx:max_col_idx])):
                        new_inflated_map[inflated_col_idx][point_idx] = self.MAP_OBSTACLE_VALUE
...
def main(args=None):
    ...
    mng = ShortPathMng(inflate_radius=0.1,) # Create the node
